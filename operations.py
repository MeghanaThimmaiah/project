...
def Create_Product(owner,product_json_file,product_ID,product_title,product_type,price_per_day,total_stock_available):
    ...
    Products={
        ...
        "Product ID": ''.join(random.choices(string.ascii_uppercase+string.digits,k=4)),
    ...
    for i in range(len(content)):
        if (content[i]["Product ID"] == product_ID):
            if detail_to_be_updated=="Product Title":
                content[i]["Product Title"] = input("enter Product Title")
                break
            elif detail_to_be_updated == "Product Type":
                content[i]["Product Type"] = input("enter Product Type")
                break
            elif detail_to_be_updated == "Price per Day":
                content[i]["Price Per Day"] = int(input("enter Price per Day"))
                break
            elif detail_to_be_updated == "Total Stock Available":
                content[i]["Total Stock Available"] = int(input("enter Total Stock Available"))
                break
            else:
                "No new update done"
                break
...
def Add_item_to_wishlist(Username,product_ID,gamers_json_file):
    ...
    try:
        ...
        Wishlist["Product ID"]=len(content+1)
        content.append(Wishlist)
...
def Remove_item_from_wishlist(Username,product_ID,gamers_json_file):
    ...
    '''Write your code below'''
    f = open(gamers_json_file_json, "r+")
    content = json.load(f)
    for i in range(len(content)):
        if content[i]["Product ID"] == product_ID:
            del content[i]
            ...
            f.close()
            return True
    return "Pls Enter Valid product ID"
